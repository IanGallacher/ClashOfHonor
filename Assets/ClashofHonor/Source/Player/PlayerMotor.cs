...
public class PlayerMotor : MonoBehaviour {
	...
	public State Move(float horizontal, float vertical, bool jump, float yaw, float pitch) {
		var moving = false;
		var movingDir = Vector3.zero;


		movingDir.z = vertical;
		movingDir.x = horizontal;

		
		if (movingDir.x != 0 || movingDir.z != 0) {
			moving = true;
			movingDir = Quaternion.Euler(0, yaw, 0) * movingDir;
		}
		...
		//this is why the camera won't rotate up and down properly
		transform.localRotation = Quaternion.Euler(0, yaw, 0);
		//we can't make our player controller follow the camera because the camera won't know which player is "owned"
		//The reason we want the player to follow the camera is so forward and back controls follow the oculus cam. 
		//We want to move in the direction that we are looking. 
		//
	//	print (gameObject.name);
	//	transform.Find ("NZZ_CameraPitch").localRotation = Quaternion.Euler (pitch, 0, 0);
